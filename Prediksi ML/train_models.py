...
# 9. Menyimpan model, scaler, dan label encoder ke file
joblib.dump(rf, os.path.join(BASE_DIR, "rf_model.pkl"))
joblib.dump(nb, os.path.join(BASE_DIR, "nb_model.pkl"))
...
def plot_metrics_bar(rf_m, nb_m):
    ...
    ax.set_ylabel("Score")
    ax.set_title("Perbandingan Metrik RF vs NB")
    ax.set_xticks(x)
...
print(f"Folder output: {STATIC_DIR}")
